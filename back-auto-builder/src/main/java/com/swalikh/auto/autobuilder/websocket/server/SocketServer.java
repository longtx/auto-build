package com.swalikh.auto.autobuilder.websocket.server;

import com.swalikh.auto.autobuilder.websocket.entity.Client;
...
public class SocketServer {
	 ...
	 */
	private final static String SYS_USERNAME = "system";
...
//				logger.info("客户端:【{}】断开连接",client.getUserName());
	...
	public synchronized static void sendMessage(String message,String userName) {
		...
		String text ="【" + dateStr + "】： " + message;
		socketServers.forEach(client ->{
			if (userName.equals(client.getUserName())) {
				try {
					client.getSession().getBasicRemote().sendText(text);
					logger.info("服务端推送给【{}】发送消息 :【{}】",client.getUserName(),message);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	public synchronized static void sendAll(String message) {
		...
		socketServers.stream()
                .filter(cli -> cli.getUserName() != SYS_USERNAME)
				.forEach(client -> {
			        try {
